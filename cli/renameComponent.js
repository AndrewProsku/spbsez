...
function renameComponentFiles(name, newName) {
    const path = `${dirPath}/${name}/${name}`;
    const pathNew = `${dirPath}/${name}/${newName}`;
...
function renameComponentDir(name, newName) {
    const path = `${dirPath}/${name}`;
    const pathNew = `${dirPath}/${newName}`;
    fs.exists(path, function(exists) {
        if(exists) {
            console.info(gutil.colors.green(`Папка ${path} переименована`));
            fs.renameSync(path, pathNew);
        } else {
            console.error(gutil.colors.red(`Папка ${path} не найдена`));
        }
    });
}
...
function renameImportScss(name, newName) {
    fs.readFile(importFile, 'utf8', (error, data) => {
        const reg = `@import "../../components/${name}/${name}";`;
        const newReg = `@import "../../components/${newName}/${newName}";`;
...
renameComponentFiles(args.name, args.newName);
renameComponentDir(args.name, args.newName);
