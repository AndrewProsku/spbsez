<?php
$MESS['KELNIK_TMPL_TITLE_SUFFIX'] = 'ОЭЗ СПб';
$MESS['KELNIK_TMPL_BACK_TO_MAIN'] = 'Вернуться на главную';
$MESS['KELNIK_TMPL_404'] = 'Такой страницы не существует';
$MESS['KELNIK_TMPL_404_TEXT'] = 'Попробуйте начать с <a href="#MAIN_PAGE#" class="b-link-line">главной страницы</a> или <a href="#SITE_PAGE#" class="b-link-line">посмотрите наши участки</a>';
$MESS['KELNIK_TMPL_FOOTER_SEZ'] = 'ОЭЗ «Санкт-Петербург»';
$MESS['KELNIK_TMPL_CABINET'] = 'Личный кабинет';
$MESS['KELNIK_TMPL_SEZ'] = 'Особая Экономическая зона';
$MESS['KELNIK_TMPL_WRITE_MESSAGES'] = 'Написать сообщение';
$MESS['KELNIK_TMPL_OLD_BROWSER_HEADER'] = 'Сайт может работать некорректно';
$MESS['KELNIK_TMPL_OLD_BROWSER_BODY'] = 'Ваш браузер устарел и не поддерживает некоторые современные технологии сайта. Чтобы работали все функции, обновите браузер или скачайте другой из проверенных:';
$MESS['KELNIK_TMPL_OLD_BROWSER_LINK'] = 'Все равно перейти на сайт';
$MESS['KELNIK_TMPL_OLD_BROWSER_YANDEX'] = 'Яндекс.Браузер';
$MESS['KELNIK_TMPL_BACK_TO_MEDIA'] = 'Назад в медиа';

$MESS['KELNIK_TMPL_FORM_COMMENT'] = 'Комментарий';
$MESS['KELNIK_TMPL_FORM_NAME'] = 'ФИО';
$MESS['KELNIK_TMPL_FORM_EMAIL'] = 'Эл. почта';
$MESS['KELNIK_TMPL_FORM_PHONE'] = 'Телефон';
$MESS['KELNIK_TMPL_FORM_SEND'] = 'Отправить';
$MESS['KELNIK_TMPL_PHONE_MASK'] = '+7 ___ ___-__-__';
$MESS['KELNIK_TMPL_PROFILE_SIGN_OUT'] = 'Выйти из личного кабинета';

$MESS['KELNIK_TMPL_SITES_LOCATION'] = 'Расположение площадок ОЭЗ';
$MESS['KELNIK_TMPL_SITES_NOVOORLOVSKAYA'] = 'Новоорловская';
$MESS['KELNIK_TMPL_SITES_NOIDORF'] = 'Нойдорф';
$MESS['KELNIK_TMPL_SITES_PARNAS'] = 'Парнас';
$MESS['KELNIK_TMPL_SITES_SHUSHARY'] = 'Шушары';
$MESS['KELNIK_TMPL_SITES_SEA_PORT'] = 'Порт';
$MESS['KELNIK_TMPL_SITES_RAILWAY_BALT'] = 'Балтийский вокзал';
$MESS['KELNIK_TMPL_SITES_CENTER'] = 'Центр города';
$MESS['KELNIK_TMPL_SITES_RAILWAY_LAD'] = 'Ладожский вокзал';
$MESS['KELNIK_TMPL_SITES_AIRPORT'] = 'Аэропорт';
$MESS['KELNIK_TMPL_SITES_MORE'] = 'Узнать больше';
$MESS['KELNIK_TMPL_SITES_SITE'] = 'Участок';
$MESS['KELNIK_TMPL_SITES_SITE_AREA'] = 'общая площадь';
$MESS['KELNIK_TMPL_SITES_SITE_AREA_HA'] = 'га';
$MESS['KELNIK_TMPL_SITES_SITE_KM'] = 'км';
$MESS['KELNIK_TMPL_SITES_TO_CENTER'] = 'от центра';
$MESS['KELNIK_TMPL_SITES_SW'] = 'юго-запад Петербурга';
$MESS['KELNIK_TMPL_SITES_STRELNA'] = 'Стрельна';
$MESS['KELNIK_TMPL_SITES_SITE_NOVOORLOVSKAYA'] = 'Площадка «Новоорловская»';
$MESS['KELNIK_TMPL_SITES_SITE_NOIDORF'] = 'Площадка «Нойдорф»';
$MESS['KELNIK_TMPL_SITES_SITE_PARNAS'] = 'Площадка «Парнас»';
$MESS['KELNIK_TMPL_SITES_SITE_SHUSHARY'] = 'Площадка «Шушары»';
$MESS['KELNIK_TMPL_SITES_DESCR_NOV'] = 'Развитая транспортная инфраструктура. Менее 10 минут до выезда на КАД.
                        30 минут до центра города на автотранспорте. 20 минут
                        до метро на автотранспорте.';
$MESS['KELNIK_TMPL_SITES_DESCR_NOIDORF'] = 'В окружении развитой городской инфраструктуры и культурного наследия. Менее
                        10 минут до выезда на КАД. 20 минут на автотранспорте до аэропорта.';
$MESS['KELNIK_TMPL_SITES_DESCR_PARNAS'] = 'В непосредственной близости от КАД и одного из крупнейших районов массового жилищного строительства с хорошей транспортной доступностью.';
$MESS['KELNIK_TMPL_SITES_DESCR_SHUSHARY'] = 'В непосредственной близости от аэропорта Пулково, а также вблизи крупных городских автомагистралей КАД и ЗСД.';
$MESS['KELNIK_TMPL_SITES_PRIM'] = '<strong>Приморский район</strong> <span>север Петербурга</span>';
$MESS['KELNIK_TMPL_SITES_VYB'] = '<strong>Выборгский район</strong> <span>север Петербурга</span>';
$MESS['KELNIK_TMPL_SITES_PUSHKIN'] = '<strong>Пушкинский район</strong> <span>юг Петербурга</span>';

$MESS['KELNIK_TMPL_CALC_TITLE'] = 'Отправить результат расчёта на email';
$MESS['KELNIK_TMPL_CHAT_TITLE'] = 'Чат с администратором';
